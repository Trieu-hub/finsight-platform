...
/**
 * The retry-exhaustion recoverer increments {@code finsight.risk.events.failed} once per record it
 * gives up on. Verified without a broker by invoking the recoverer directly — the same callback
 * {@code DefaultErrorHandler} runs after the back-off is exhausted (RL-1). Mirrors budget-service's
 * KafkaConsumerConfigTest.
 */
class KafkaConsumerConfigTest {

    private static final String FAILED = "finsight.risk.events.failed";
