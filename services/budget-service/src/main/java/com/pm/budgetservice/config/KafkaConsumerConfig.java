 ...
 * {@code finsight.budget.events.failed} via the recoverer, which the error handler
 ...
 * {@code failed} means the event should have been applied but never could be, so it is
 * the one counter that signals real data loss.
 */
@Configuration
...
public class KafkaConsumerConfig {
    ...
    static ConsumerRecordRecoverer failedEventRecoverer(MeterRegistry meterRegistry) {
        Counter failedEvents = Counter.builder("finsight.budget.events.failed")
                .description("TransactionCreated records skipped after retries were exhausted "
                        + "(poison message or persistent failure — signals dropped spend)")
