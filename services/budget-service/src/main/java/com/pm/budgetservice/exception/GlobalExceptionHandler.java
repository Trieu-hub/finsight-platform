...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleNotFound(BudgetNotFoundException ex) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
                .body(new ErrorResponse("BUDGET_NOT_FOUND", ex.getMessage()));
    }

    @ExceptionHandler(BudgetConflictException.class)
    public ResponseEntity<ErrorResponse> handleConflict(BudgetConflictException ex) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body(new ErrorResponse("BUDGET_ALREADY_EXISTS", ex.getMessage()));
    }

    @ExceptionHandler(InvalidBudgetDataException.class)
    public ResponseEntity<ErrorResponse> handleInvalidData(InvalidBudgetDataException ex) {
